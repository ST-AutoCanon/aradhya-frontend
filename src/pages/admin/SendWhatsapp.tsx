...
// 📁 src/pages/admin/SendWhatsapp.tsx
...
const SendWhatsapp: React.FC<SendWhatsappProps> = ({
  isOpen,
  onClose,
  recipients,
  serviceName,
  ...
  const sendWhatsapp = () => {
    ...
    try {
      // Open WhatsApp window for each recipient
      ...
      alert("WhatsApp message window(s) opened!");
      handleClose();
    } catch (err) {
      console.error(err);
      alert("Error sending WhatsApp messages");
    } finally {
      setSending(false);
    }
  };
